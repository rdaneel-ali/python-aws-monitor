    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if response.status_code == 200:
                logging.info(
                    f"✅ {name} - SUCCESS ({response.status_code}) on attempt {attempt}"
                )
                return True, f"Success: {response.status_code}"
            else:
                logging.warning(
                    "⚠️  %s - Unexpected status %s - attempt %d/%d",
                    name,
            ...
            logging.error(
                f"❌ {name} - Timeout after {timeout}s - attempt {attempt}/{retries}"
            )
        except requests.exceptions.ConnectionError:
            logging.error(f"❌ {name} - Connection error - attempt {attempt}/{retries}")
        except requests.exceptions.RequestException as e:
            logging.error(
                f"❌ {name} - Request failed: {str(e)} - attempt {attempt}/{retries}"
            )
        except Exception as e:
            logging.error(
                f"❌ {name} - Unexpected error: {str(e)} - attempt {attempt}/{retries}"
            )
...
def main(args=None):
    ...
    logging.info(
        f"Configuration: timeout={timeout}s, retries={retries}, delay={delay}s"
    )
    ...
    for endpoint in endpoints:
        try:
            ...
            # Store results
            results.append({"name": name, "url": url, "success": ok, "message": info})
