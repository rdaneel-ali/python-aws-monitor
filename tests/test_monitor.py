...
def test_check_url_success():
    ...
    ok, status = monitor.check_url(
        "https://httpbin.org/status/200", "Test Success", timeout=10, retries=1
    )
    assert ok is True
...
def test_check_url_failure():
    ...
    ok, status = monitor.check_url(
        "https://httpbin.org/status/404", "Test Failure", timeout=10, retries=1
    )
    assert ok is False
...
def test_check_url_timeout():
    ...
    ok, status = monitor.check_url(
        "https://httpbin.org/delay/10", "Test Timeout", timeout=2, retries=1
    )
    assert ok is False
    assert "Failed after" in status
